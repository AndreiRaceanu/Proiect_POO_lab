...
void Game::setup()
{
    ...
    while(true)
    {
        ...
        C->City = Copy[x].City;
        C->Debt = Copy[x].Debt;
        C->Given_name = Copy[x].Given_name;
        C->Surname = Copy[x].Surname;
        C->Weight = Copy[x].Weight;
...
int main()
{
    ...
    for(auto it = G1.Remainders2.begin();it!=G1.Remainders2.end();it++)
    {
        if(it->second.size()==0)
        {
            ...
            {
                G1.V[i].first.suma = G1.V[i].first.suma + j->Debt;
            }
            G1.V[i].first.suma = G1.V[i].first.suma - G1.V[i].first.Debt;
            G1.Supervisors_with_money.push_back(G1.V[i].first);
        }
        if(it->second.size()==1)
        {
            G1.V[i].first.suma = G1.Competitors2.front().Debt * 10 - G1.V[i].first.Debt;
            G1.Supervisors_with_money.push_back(G1.V[i].first);
        }
        i++;
    }
    ...
    {
        cout<<it->City<<" "<<it->Debt<<" "<<it->Given_name<<" "<<it->Mask_shape<<" "<<it->Surname<<" "<<it->Weight<<" With the gain "<<it->suma<<"\n";
    }
    int s1,s2,s3=0;
